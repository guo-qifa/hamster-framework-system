...
export default {
  ...
  data() {
    return {
      form: { name: "众力", password: "1", code: "32" },
      submitLoading: false,
    };
  },
  methods: {
    onSubmit(v) {
      ...
      this.$bubble
        ...
        .then((v) => {
          if (v.code == 0) {
            that.$bubble.request("userType.find", {}).then((r) => {
              this.$store.commit("setUserType", r.record.userBaseVO);
              localStorage.setItem('orgId', r.record.userBaseVO.orgId);
              localStorage.setItem('userName', r.record.userBaseVO.userName);
              localStorage.setItem('userType', r.record.userBaseVO.userType);
              this.$router.push("/");
            });
          } else {
            this.$alert(v.message);
          }
        })
        .catch((v) => {
          this.$alert("登录失败" + " [" + v + "]");
