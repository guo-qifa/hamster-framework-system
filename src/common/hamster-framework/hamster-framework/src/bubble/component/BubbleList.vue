<template>
  <div style="overflow: hidden" class="list-component-box" v-loading="loading">
    ...
    <el-table
      ...
      :highlight-current-row="selectMode == 'single' && !!this.dataKey"
    ...
    >
      ...
      <el-table-column
        width="55"
        v-if="selectMode == 'multiple' && this.dataKey"
...
export default {
  ...
  computed: {
    config: function () {
      if (this.tableConfig) {
        ...
        return this.$bubble.TableConfig[this.type];
      }
    },
    rowBtnAuth: function () {
      var rs = [];
  ...
  methods: {
    ...
    getColText(v, i) {
      ...
      if (i.remote) {
        //  return this.$bubble.Dictionary[i.remote][v];
      }
    ...
    getRemoteDict(v) {
        var _this = this 
      //system/dict/selectByCode?dictCode=wxyxzdj
      debugger
      console.log(this)
      debugger
      var ajaxPromise = axios.get(
         "system/dict/selectByCode?dictCode="+ v,
        { 
          headers: {
            loginTokensAnjian: "Bearer" + JSON.parse(localStorage.getItem("bubbleCache")).token
          },
        }
      );
      ajaxPromise
    ...
    initRemoteSelectData(v) {
      ...
      this.$bubble.request(v.url, v.parame).then((data) => {
        ...
        _this.$set(
          ...
          v.url,
          data.map((tmp) => {
            return { text: tmp[v.text], value: tmp[v.value] };
          })
        );
        _this.$set(_this.searchForm, [item.key], data[0][v.value]);
        _this.$nextTick(() => {
          _this.$delete(_this.searchForm, [item.key]);
        });
      });
    },
  ...
  created: function () {
    if (this.remotedict) {
      let remotedict = this.remotedict.split(","); 
      for (var j = 0; j < remotedict.length; j++) {
        this.getRemoteDict(remotedict[j]);
      }
    }
    if (this.search) {
      for (var i = 0; i < this.search.length; i++) {
